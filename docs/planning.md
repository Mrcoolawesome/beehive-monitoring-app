...
- **`upsert` instead of `create`, plus a unique `(piMacAddress, timestamp)` constraint (found during first real server deployment):** running `watcher` and `../beehive-project`'s `decoder` against the same live `DpCat` simultaneously (rather than one at a time, as in local testing) revealed a real race between them: `watch_dpcat_decode.py` used "does my `.json` output still exist next to the `.fdp`" as its only "already decoded" signal, but `watcher` *moves* that `.json` into `processed/` once ingested — so decoder immediately saw its own output as missing again and redecoded, which watcher then re-ingested, forever, in lockstep. Fixed at the source in `watch_dpcat_decode.py` (tracks what it's already decoded in memory instead of trusting the filesystem), but `scripts/watcher.ts` also switched from `create` to `upsert` on `(piMacAddress, timestamp)` as a second line of defense — the same reading getting handed to the watcher twice, from any cause, now overwrites the same row instead of creating a duplicate. 187 duplicate rows this loop created in the few minutes before it was caught had to be truncated from the server's database.
